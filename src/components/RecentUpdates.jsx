...
function SmallCard({ item, index, anyHovered, onHover, onLeave }) {
  return (
    <Link
      to={'/blog/' + item.slug}
      className="lift-card group relative overflow-hidden block rounded-[1.5rem] border border-white/[0.08] transition-opacity duration-500"
...
export default function RecentUpdates() {
  ...
  return (
    <section className="relative w-full py-24 md:py-32 px-5 overflow-hidden">
      <div className="absolute left-0 top-1/3 h-80 w-80 rounded-full bg-cyan-500/10 blur-3xl pointer-events-none" />
      <div className="relative max-w-6xl mx-auto">
        ...
        >
          <p className="inline-flex rounded-full border border-indigo-300/15 bg-indigo-300/[0.06] px-3 py-1.5 text-[11px] font-semibold tracking-[0.2em] uppercase text-indigo-300/80 mb-4">Updates</p>
          <div className="flex items-end justify-between gap-4 flex-wrap">
            ...
            <Link
              to="/blog"
              className="chip-hover rounded-full border border-white/[0.08] bg-white/[0.035] px-4 py-2 text-[13px] font-medium text-slate-400 hover:text-white transition-colors flex items-center gap-1.5 mb-1"
            >
              View all
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Link
            to={'/blog/' + latest.slug}
            className="lift-card group relative overflow-hidden block rounded-[1.75rem] border border-white/[0.08] transition-opacity duration-500"
